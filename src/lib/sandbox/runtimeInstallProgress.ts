...
// 它不是 SandboxDto 的一部分，只喂进度卡「启动运行环境」格子下的一行子文案。
// 存在的理由：装 CLI 期间 sandbox.status 恒为 `starting`，而 AIO 默认镜像上现装 claude-code
// 实测 **753 秒（12.5 分钟）**（04 §3 ★1）——没有子文案，用户盯着「启动运行环境」干等 12 分钟，看起来像卡死。
...
/**
 * 「启动运行环境」格下的子文案。返回 undefined = 本格不加子文案（进度卡照常渲染四格）。
 *
...
export function installSubCopy(progress: RuntimeInstallProgress | undefined): string | undefined {
  ...
  switch (progress.status) {
    case 'not_installed':
      return `镜像里没有预装 ${progress.runtime} 的命令行工具，正在准备安装…`;
    case 'installing':
      // 明确写出"可能十几分钟、不是卡死"——P22 §1 INSTALL_FAILED 行的说明要求。
      return `正在安装 ${progress.runtime} 的命令行工具…（这张镜像里没有预装它，现装可能要十几分钟，不是卡死）`;
    case 'installed':
      return progress.versionDetected !== undefined && progress.versionDetected !== ''
        ? `${progress.runtime} 的命令行工具已就绪（${progress.versionDetected}）`
        : `${progress.runtime} 的命令行工具已就绪`;
    case 'failed':
      return undefined;
  }
}
