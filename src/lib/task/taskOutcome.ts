...
const TASK_ERROR_COPY = {
  ...
  // —— 平台侧恢复路径 ——
  SANDBOX_GONE:
    '任务跑到一半，它的运行环境没了（被停止、被删除，或者自己退出了），这一轮到此为止。确认运行环境还在之后再重跑。',
  RESUME_FAILED:
   ...
   */
  UNKNOWN_RUNTIME:
    '这个任务用的 Agent 已经不在平台的注册表里，这一轮到此为止。常见于第三方模块带进来的 Agent：平台重启之后那个模块没有再加载。重跑同一个 Agent 只会再失败一次——装回那个模块，或者改用注册表里还在的 Agent 重新发起。',
  // —— provider 契约错误里"码本身就说清了"的那些 ——
  IMAGE_PULL_FAILED: '没能把镜像拉下来，任务没跑起来。确认镜像名与镜像仓库凭证之后重试。',
  /**
   ...
   */
  // ⚠️ 这里**不许写 `**重点**`**：`TaskOutcome.view` 是 `<p>{copy.advice}</p>`，
  //    全仓没有 markdown 渲染器，星号会原样上屏。要强调就把重点放句首。
  IMAGE_DIGEST_GONE:
    '重跑没有用：这个任务要用的那一版镜像，仓库里已经没有了。镜像地址没有写错——平台按注册时锁定的那一版去拉，而上游把它删掉或者回收了。到设置 → 镜像管理对这张镜像点[检查更新]，换到新版本之后再重新发起。',
  RESOURCE_EXHAUSTED: '资源暂时不够，任务没能跑完。停掉几个不用的任务把资源让出来，再重试。',
  // ⚠️ 超时 ≠ 不可达：这里只知道"没在限定时间内做完"，别写成"连不上"。
  TIMEOUT:
    '平台对容器服务的一次操作等太久，任务被中断了（这不是你设的那个硬超时）。超时不等于连不上，可以稍后重试；一直这样就去看系统状态。',
  PROVIDER_UNAVAILABLE:
    '容器服务没有响应，任务被中断了。先确认 Docker Desktop 或 OrbStack 这类容器服务已经在运行，再重试。',
  // ⚠️ 「运行档位」这个词已从界面退休（选择权收回后端，界面上没有这个开关）——
  //    ⛔ 不许再让用户去改一个不存在的下拉。
  UNSUPPORTED_CAPABILITY:
    '这台机器的沙箱环境不支持无头运行（不开终端、直接跑完的那种）。换一台支持它的机器重新发起，或者改用交互式终端。',
  INTERNAL: '平台内部出错，任务被中断了。可以重跑一次；一直失败就去看系统状态。',
