...
export const Enabled: Story = {
  play: async ({ canvasElement }) => {
    ...
    // MUTATION：把 `LifecycleIcon` 换回 emoji 或换成另一个图标 ⇒ 下面两条先红。
    const icon = canvas.getByTestId('detail-lifecycle-icon');
    await expect(icon).toHaveAttribute('data-lifecycle-status', 'ok');
    await expect(icon.classList.contains('lucide-check')).toBe(true);
  },
};

/** 自动禁用（`status: 'fail'`）：按钮说的是 [重新启用]。 */
export const AutoDisabled: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.getByTestId('detail-toggle')).toHaveTextContent('重新开启');
    const icon = canvas.getByTestId('detail-lifecycle-icon');
