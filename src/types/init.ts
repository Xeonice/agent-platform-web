...
export interface PresetImageStepModel {
  ...
  /** 这一步在检查什么。 */
  label: string;
  /** 后端对这一步的**一句话结论**（只有走到的那一步才有）。≤ 20 字，第一眼就看得懂。 */
  summary?: string;
  /**
   * 结论的证据 / 例外条款 / 为什么 —— 第二层，渲染在结论下面一行、弱化。
   *
   * ⛔ 它与 `summary` 是两个字段。合成一个时那句话长成三行散文，而它渲染在
   * 「第 N 步」标题下面 —— 用户要读完一整段才知道这一步到底过没过。
   */
  detail?: string;
  /**
   ...
   */
  action?: string;
  /** 可复制的修复命令（优先用后端 `command`：它带着这台机器上的真实取值）。 */
  fixCommand?: string;
  errorCode?: string;
  /**
